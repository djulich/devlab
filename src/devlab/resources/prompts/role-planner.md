...
- Declare external session, setup, or validation prerequisites in the owning profile when they can be known during planning. Use non-mutating automatic checks where possible, operator attestations only for uncheckable authority or conditions, and a detailed repository-owned remediation guide when a short summary is insufficient. Do not encode role lists; prerequisites follow the profile operation that consumes them.
