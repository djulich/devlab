...
- Declare external session, setup, or validation prerequisites in the owning profile when they can be known during planning. Use non-mutating automatic checks where possible and operator attestations only for uncheckable authority. When a short summary is insufficient, add a focused repository-owned resolution guide—not a broad prerequisite inventory—that covers the required capability, installation or provisioning boundary, configuration, exact readiness check, common failures, and applicable security or cleanup constraints. Use a dedicated file or `path.md#heading-slug`, and prefer authoritative upstream links over copied volatile instructions. Do not encode role lists; prerequisites follow the profile operation that consumes them.
