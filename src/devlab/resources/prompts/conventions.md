...
Profiles may declare `[[prerequisites]]` for external conditions required by `session`, `setup`, or `validation` operations. Prefer fast, non-mutating `check` commands or environment-variable presence checks. Use an operator `attestation` only for authority or conditions DevLab cannot safely verify. Provide a concise `summary` and, when resolution is not obvious, a focused repository-owned `guide` using a dedicated file or `path.md#heading-slug`. A guide must explain how to establish and verify the capability, not merely list it; include configuration, common failures, and security or cleanup constraints when applicable. Prefer authoritative upstream links for volatile installation instructions. Prerequisites follow the consuming profile operation; do not scope them to role names. DevLab checks and reports prerequisites but does not install tools, start services, provision databases, or obtain credentials.
