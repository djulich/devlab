...
7. **Compiled-language profiles** — opt-in Rust, Go, C, and C++/CMake live scenarios exercise operator-provided toolchains, dedicated task profiles, inherited validation, and target-owned black-box behavior. The initial Rust/C++ baseline is recorded in `docs/plans/compiled-language-live-baseline-2026-08-20.md`; collect Go/C outcomes before considering broader toolchain variants.
...
- Collect initial outcomes for `live-go-cli-happy-path` and `live-c-cmake-cli-happy-path` on hosts with the complete operator-installed toolchains.
